...
document.addEventListener("DOMContentLoaded", function () {
    ...
    async function formSend(e) {
        ...
        if (error === 0) {
            ...
            // * <_ вот это просто для галочки, я в этом не шарю, НО, хотелось бы _> =================================
            let response = await fetch('sendmail.php', {
                method: 'POST',
                body: formData
            });

            // * <_ это тоже не знаю _> =================================
            if (response.ok) {
...
let scr_items = document.querySelectorAll('._scr-item'); // * <_ Добавляет класс "_active" к элементу _> =================================
	...
	if (scr_items.length > 0) {
		for (let index = 0; index < scr_items.length; index++) {
			...
			if ((src_value > scr_item_offset - scr_item_point) && src_value < (scr_item_offset + scr_item_height)) {
				scr_item.classList.add('_scr--active');
				scroll_load_item(scr_item);
			} else {
				scr_item.classList.remove('_scr--active');
			}
...
//
// todo <_ ScrollOnClick (Navigation) _> =========================================================================,
//
